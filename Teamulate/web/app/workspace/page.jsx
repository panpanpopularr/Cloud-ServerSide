...
export default function Page() {
  ...
  const renderActivity = (a) => {
    ...
    if (a.type === 'TASK_DELETED') {
      return (
        <>
          <div style={{ fontWeight: 700, color: '#fca5a5' }}>Delete task</div>
          <div>ชื่อ/ID: {taskTitleById(p.taskId)}</div>
          <div style={{ fontSize: 12, opacity: .7 }}>โดย {by} · {date} {time}</div>
        </>
      );
    }
    if (a.type === 'FILE_UPLOADED')
      return (<><div style={{ fontWeight: 600 }}>Upload file</div><div>ชื่อ: {p.name ?? '(unknown)'}</div><div style={{ fontSize: 12, opacity: .7 }}>โดย {by} · {date} {time}</div></>);
    if (a.type === 'FILE_DELETED')
      return (<><div style={{ fontWeight: 600 }}>Delete file</div><div>ชื่อ: {p.name ?? '(unknown)'}</div><div style={{ fontSize: 12, opacity: .7 }}>โดย {by} · {date} {time}</div></>);
    if (a.type === 'PROJECT_CREATED')
      return (<><div style={{ fontWeight: 600 }}>Create project</div><div>ชื่อ: {p.name ?? '(no-name)'}</div><div style={{ fontSize: 12, opacity: .7 }}>โดย {by} · {date} {time}</div></>);
    if (a.type === 'MEMBER_ADDED')
      return (<><div style={{ fontWeight: 600 }}>Add member</div><div>userId: {p.userId}</div><div style={{ fontSize: 12, opacity: .7 }}>โดย {by} · {date} {time}</div></>);
    if (a.type === 'MEMBER_REMOVED')
      return (<><div style={{ fontWeight: 600 }}>Remove member</div><div>userId: {p.userId}</div><div style={{ fontSize: 12, opacity: .7 }}>โดย {by} · {date} {time}</div></>);
    return (
      <>
        ...
        <div style={{ fontSize: 11, opacity: .6 }}>โดย {by} · {date} {time}</div>
      </>
    );
  };
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr 380px', gap: 16 }}>
      ...
      {/* Activity (ทำให้เลื่อนแยกได้) */}
      <div
        style={{
          ...card,
          /* สูงสุดตาม viewport และเลื่อนเฉพาะการ์ดนี้ */
          maxHeight: 'calc(100vh - 140px)', // ปรับได้ตามดีไซน์ header/padding
        }}
      >
